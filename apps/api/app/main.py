...
# local-disk mode. Once R2 is configured, get_storage() returns S3Storage
# and every media URL already points straight at the R2 public bucket, so
# there is nothing under media_root to serve, and mounting it unconditionally
# used to crash startup here: S3Storage's __init__ never creates that
# (unused) directory the way LocalDiskStorage's does, so StaticFiles found
# nothing to mount on a container that had never run in local-disk mode.
# media_base_url in config.py must stay in sync with this mount path when
# it is actually used.
get_storage()  # Ensures media_root exists before StaticFiles tries to serve from it, in local-disk mode.
